...
async def main():
    ...
    with open('user_ids.txt', 'r') as user_file:
        user_ids = [line.strip() for line in user_file if line.strip()]

    if not user_ids:
        print(f"{Fore.RED}No user IDs found in user_ids.txt! Exiting.{Style.RESET_ALL}")
        return

    with open('proxy_list.txt', 'r') as proxy_file:
        local_proxies = [line.strip() for line in proxy_file if line.strip()]

    if not local_proxies:
        print(f"{Fore.RED}No proxies found in proxy_list.txt! Exiting.{Style.RESET_ALL}")
        return

    proxies_per_user = int(input("Enter the number of proxies per user: ").strip())

    tasks = []
    for user_id in user_ids:
        assigned_proxies = random.sample(local_proxies, min(proxies_per_user, len(local_proxies)))
        for proxy in assigned_proxies:
            tasks.append(connect_to_wss(proxy, user_id, mode))

    print(f"{Fore.GREEN}Starting {len(tasks)} tasks...{Style.RESET_ALL}")
